...
# Study completion date from Overall study end date column. 
#Change date format to month & year
  
mutate(`Overall study end date`=format(ymd(`Overall study end date`),"%B %Y")) %>%
mutate(`Overall study end date` = str_replace(`Overall study end date`, "^", "**Study completion date:** ")) %>%
...
df<-select(df,Title,ISRCTN,`EudraCT/CTIS number`,Phase,MoP,`Age group`,`Target number of participants`,Primary_outcomes, `Overall study status`, `Overall study start date`,`Primary completion date`,`Overall study end date`,`Basic results...9`, blank)
