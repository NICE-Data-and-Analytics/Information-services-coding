...
# e.g. NG215 will become "NICE guideline NG215"
  ...
  mutate(`Reference number` = str_replace_all(`Reference number`, "^PH", "NICE guideline PH"))%>%
...
df <- df %>%
  mutate(
    # Extract both dates using regular expressions
    date1 = as.Date(str_extract(Merged, "\\(20\\d{2}\\)"), format = "(%Y)"),
    date2 = as.Date(str_extract(Merged, "(?<=updated )20\\d{2}"), format = "%Y"),
    # Determine the later date
    later_date = pmax(date1, date2, na.rm = TRUE)
  ) %>%
  select(Merged, later_date) %>%
  arrange(desc(later_date))
